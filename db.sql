...
CREATE TABLE PRODUCT_LOGS (
    ID int primary key auto_increment,
    PRODUCT_ID int NOT NULL,
    ACTION_TYPE enum('ADD', 'DELETE', 'EDIT') NOT NULL,
    ACTION_TIME TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    DESCRIPTION varchar(255) NOT NULL,
    FOREIGN KEY (PRODUCT_ID) REFERENCES PRODUCTS(ID)
);
...
INSERT INTO ORDERS (PRODUCT_ID, USER_ID, QUANTITY, STATE) VALUES
(1, 1, 2, 'Completed'),
(2, 2, 1, 'Pending'),
(3, 3, 5, 'Cancelled'),
(4, 4, 3, 'Completed'),
...
-- Trigger for logging product additions
DELIMITER //
CREATE TRIGGER after_product_insert
AFTER INSERT ON PRODUCTS
FOR EACH ROW
BEGIN
    INSERT INTO PRODUCT_LOGS (PRODUCT_ID, ACTION_TYPE, DESCRIPTION)
    VALUES (NEW.ID, 'ADD', CONCAT('Product ', NEW.PRODUCTNAME, ' added.'));
END;
//
DELIMITER ;

-- Trigger for logging product deletions
DELIMITER //
...
BEGIN
    INSERT INTO PRODUCT_LOGS (PRODUCT_ID, ACTION_TYPE, DESCRIPTION)
    VALUES (OLD.ID, 'DELETE', CONCAT('Product ', OLD.PRODUCTNAME, ' deleted.'));
END;
//
DELIMITER ;
...
BEGIN
    INSERT INTO PRODUCT_LOGS (PRODUCT_ID, ACTION_TYPE, DESCRIPTION)
    VALUES (NEW.ID, 'EDIT', CONCAT('Product ', NEW.PRODUCTNAME, ' updated.'));
END;
//
DELIMITER ;